...
const Texture* Postprocess::GaussianBlur(PPToolbox<GaussianBlurToolbox> tb, const GEE_FB::Framebuffer& writeFramebuffer, const Viewport* viewport, const Texture* tex, int passes, unsigned int writeColorBuffer) const	//Implemented using ping-pong framebuffers (one gaussian blur pass is separable into two lower-cost passes) and linear filtering for performance
{
	...
	for (int i = 0; i < passes; i++)
	{
		...
		{
			tb.GetTb().BlurFramebuffers[horizontal]->Bind();
			const Texture* bindTex = (i == 0) ? (tex) : (tb.GetTb().BlurFramebuffers[!horizontal]->GetColorBuffer(0).get());
			bindTex->Bind();
		}
		tb.GetTb().GaussianBlurShader->Uniform1i("horizontal", horizontal);
		tb.RenderFullscreenQuad(tb.GetTb().GaussianBlurShader);
	...
	tb.GetTb().BlurFramebuffers[!horizontal]->GetColorBuffer(0).get();
...
const Texture* Postprocess::SSAOPass(RenderInfo& info, const Texture* gPosition, const Texture* gNormal)
{
	...
	RenderFullscreenQuad(info.TbCollection, tb->SSAOShader);
...
const Texture* Postprocess::TonemapGammaPass(PPToolbox<ComposedImageStorageToolbox> tb, const GEE_FB::Framebuffer& writeFramebuffer, const Viewport* viewport, const Texture* colorTex, const Texture* blurTex) const
{
	...
	Shader* tonemapGammaShader = tb.GetTb().TonemapGammaShader;
	...
	tb.RenderFullscreenQuad(tonemapGammaShader);
