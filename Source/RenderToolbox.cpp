...
void SSAOToolbox::Setup(const GameSettings::VideoSettings& settings)
{
	...
	SSAONoiseTex = std::make_shared<Texture>();
...
void SMAAToolbox::Setup(const GameSettings::VideoSettings& settings)
{
	...
	//////////////////////////////TEX GENERATION//////////////////////////////
	SMAAAreaTex = std::make_shared<Texture>(Texture(textureFromBuffer(areaTexBytes, AREATEX_WIDTH, AREATEX_HEIGHT, GL_RG, GL_RG, GL_UNSIGNED_BYTE, GL_NEAREST, GL_NEAREST)));
	SMAASearchTex = std::make_shared<Texture>(Texture(textureFromBuffer(searchTexBytes, SEARCHTEX_WIDTH, SEARCHTEX_HEIGHT, GL_R8, GL_RED, GL_UNSIGNED_BYTE, GL_NEAREST, GL_NEAREST)));
}
...
void ComposedImageStorageToolbox::Setup(const GameSettings::VideoSettings& settings)
{
	...
	TonemapGammaShader = AddShader(ShaderLoader::LoadShadersWithInclData("TonemapGamma", settings.GetShaderDefines(settings.Resolution), "Shaders/tonemap_gamma.vs", "Shaders/tonemap_gamma.fs"));
