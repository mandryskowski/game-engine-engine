...
void GameEngineEngineEditor::LoadProject(const std::string& filepath)
{
	...
	/*struct ProjectData
	{
		...
		ProjectData(const std::string& filepath, const std::string& filename) : ()
	};*/
	std::vector<std::string> filepaths;
	{
		...
		std::string line;
		while (std::getline(recentProjectsInput, line) && filepaths.size() < 4 && line != filepath)	//do not go past the limit of 4 filepaths
			filepaths.push_back(line);
