...
{
	vec4 projCoords = light.lightProjection * vec4(fragPosition, 1.0);
...
vec3 CalcLight(Light light, Fragment frag)
{
	...
	float intensity = 1.0;
	float alignment = dot(-lightDir, normalize(light.direction.xyz));
	intensity = clamp((alignment - light.outerCutOff) / (light.cutOff - light.outerCutOff), 0.0, 1.0);
