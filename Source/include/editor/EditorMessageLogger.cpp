	...
	namespace Editor
	{
			...
			QueuedMessagesText(nullptr)
		{
			if (IconsMaterial = dynamic_cast<AtlasMaterial*>(EditorHandle.GetGameHandle()->GetRenderEngineHandle()->FindMaterial("GEE_E_Message_Icons").get()); !IconsMaterial)
			{
				...
				IconsMaterial->AddTexture(NamedTexture(Texture::Loader<>::FromFile2D("EditorAssets/messageicons.png", Texture::Format::RGBA(), true), "albedo1"));
		...
		void EditorMessageLogger::Log(const std::string& message, MessageType type)
		{
			...
			content.CreateComponent<TextConstantSizeComponent>("GEE_E_LogText", Transform(Vec2f(-0.8f, 0.0f), Vec2f(1.6f, 2.0f)), message, "", std::pair<TextAlignment, TextAlignment>(TextAlignment::LEFT, TextAlignment::CENTER));// .Unstretch();
