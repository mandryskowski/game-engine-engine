...
public:
	...
	void RenderFullscreenQuad(Shader* shader) { PostprocessRef.RenderFullscreenQuad(TbCollection, shader); }	//Pass nullptr to render with default Quad shader (which just renders the texture at slot 0)
...
public:
	...
	void RenderFullscreenQuad(RenderToolboxCollection& tbCollection, Shader* shader = nullptr) const { RenderInfo info(tbCollection); if (!shader) shader = RenderHandle->FindShader("Quad");  RenderHandle->RenderStaticMesh(info, MeshInstance(RenderHandle->GetBasicShapeMesh(EngineBasicShape::QUAD)), Transform(), shader); }
