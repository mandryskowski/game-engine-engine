...
void ComponentDefaultDeleter::operator()(Component* comp)
{
	delete comp;
}
...
{
	RootComponent = UniquePtr<Component, ComponentDefaultDeleter>(new Component(*this, nullptr, Name + "'s root", t));
}
...
{
	std::cout << "ACTOR MOVE CONSTRUCTOR. (" + Name + ") (" + Scene.GetName() + ")\n";
	RootComponent = UniquePtr<Component, ComponentDefaultDeleter>(new Component(*this, nullptr, Name + "'s root", Transform()));
}
...
std::vector<Actor*> Actor::GetChildren()
{
	...
	std::transform(Children.begin(), Children.end(), std::back_inserter(children), [](UniquePtr<Actor>& child) -> Actor*
	{
		return child.get();
	});
...
void Actor::ReplaceRoot(UniquePtr<Component> component)
{
	...
	RootComponent = UniquePtr<Component, ComponentDefaultDeleter>(component.release());
}
...
void Actor::HandleEventAll(const Event& ev)
{
	...
	for (int i = 0; i < childrenCount; i++)
		//This type of loop is put here on purpose. The children might add other children, which would case iterators to become invalid.
...
void Actor::MarkAsKilled()
{
	...
	if (!RootComponent->IsBeingKilled())
		//Check if the root is not being killed to avoid infinite recursion (killing the root kills the actor which kills the root which kills the actor...)
...
void Actor::Delete()
{
	...
	if (!ParentActor)
		std::cout << "INFO: Cannot delete actor " + GetName() +
			" because it doesn't have a parent (it is probably the root of a scene). Hopefully it will be deleted when the scene gets deleted.\n";
	else
		ParentActor->Children.erase(std::remove_if(ParentActor->Children.begin(), ParentActor->Children.end(),
		                                           [this](UniquePtr<Actor>& child) { return child.get() == this; }),
		                            ParentActor->Children.end());
}
...
void Actor::GetEditorDescription(EditorDescriptionBuilder descBuilder)
{
	...
	UIButtonActor& deleteButton = deleteField.CreateChild<UIButtonActor>(
		"DeleteButton", "Delete", [this, descBuilder]() mutable
		{
			MarkAsKilled();
			descBuilder.RefreshScene();
			//Actor will be deselected automatically - SelectActor(nullptr) is always called in SelectScene(...).
		});

	if (Scene.GetRootActor() == this) //Disallow deleting the root of a scene
		deleteButton.SetDisableInput(true);

	descBuilder.AddField("Set parent to").GetTemplates().ObjectInput<Actor, Actor>(
		*Scene.GetRootActor(), [this](Actor* newParent) { if (newParent) PassToDifferentParent(*newParent); },
		ParentActor);
}

template <typename Archive>
void Actor::Save(Archive& archive) const
{
	archive(CEREAL_NVP(Name), CEREAL_NVP(ActorGEEID), CEREAL_NVP(RootComponent), CEREAL_NVP(Children));
}

template <typename Archive>
void Actor::Load(Archive& archive)
{
...
GEE_SERIALIZATION_INST_DEFAULT(GEE::Actor);