...
namespace GEE
{
	...
	using RenderingContextID = unsigned int;
	...
	*/
	...
	{
	public:
		MatrixInfo(const Mat4f& view = Mat4f(1.0f), const Mat4f& projection = Mat4f(1.0f),
		           const Vec3f& camPos = Vec3f(0.0f), unsigned int contextID = 0) : CamPosition(camPos), View(view),
			Projection(projection), VP(projection * view), ContextID(contextID)
		{
		}

		MatrixInfo(unsigned int contextID, const Mat4f& view = Mat4f(1.0f), const Mat4f& projection = Mat4f(1.0f),
		           const Vec3f& camPos = Vec3f(0.0f)) : CamPosition(camPos), View(view), Projection(projection),
		                                                VP(projection * view), ContextID(contextID)
		{
		}
		 ...
		 * @param proj 
		*/
		void SetVPArtificially(const Mat4f& vp)
		{
			View = vp;
			Projection = Mat4f(1.0f);
			CalculateVP();
		}
	...
	*/
	...
	public:
		...
		MatrixInfoExt(const MatrixInfoExt& info)
			: MatrixInfo(info), bUseMaterials(info.bUseMaterials), bOnlyShadowCasters(info.bOnlyShadowCasters),
			  bMainPass(info.bMainPass), bAllowBlending(info.bAllowBlending),
			  RequiredShaderInfo(info.RequiredShaderInfo)
		{
		}
		...
		MatrixInfoExt(RenderingContextID contextID, const Mat4f& view = Mat4f(1.0f),
		              const Mat4f& projection = Mat4f(1.0f), const Vec3f& camPos = Vec3f(0.0f))
			: MatrixInfo(view, projection, camPos, contextID), RequiredShaderInfo(MaterialShaderHint::None)
		{
			DefaultBools();
		}

		MatrixInfoExt(const Mat4f& view = Mat4f(1.0f), const Mat4f& projection = Mat4f(1.0f),
		              const Vec3f& camPos = Vec3f(0.0f))
		...
		void SetRequiredShaderInfo(ShaderInfo info);
		void StopRequiringShaderInfo();
		...
		ShaderInfo GetRequiredShaderInfo() const { return RequiredShaderInfo; }

	private:
		bool bUseMaterials, bOnlyShadowCasters, bMainPass, bAllowBlending;
		ShaderInfo RequiredShaderInfo;
	};
	...
	*/
	...
	{
	public:
		SceneMatrixInfo(RenderingContextID contextID, RenderToolboxCollection& tbCollection,
		                GameSceneRenderData& sceneRenderData,
		                const Mat4f& view = Mat4f(1.0f), const Mat4f& projection = Mat4f(1.0f),
		                const Vec3f& camPos = Vec3f(0.0f))
			: MatrixInfoExt(contextID, view, projection, camPos), TbCollection(tbCollection),
	...
	public:
		...
		TbInfo(RenderToolboxCollection& tbCollection, const InfoType& info)
			: InfoType(info), TbCollection(tbCollection)
		{
		}
